...
public class IotdbDsProvider extends DefaultJdbcProvider {
    ...
    public List<String[]> getData(DatasourceRequest dsr) throws Exception {
        ...
        try (Connection connection = getConnectionFromPool(dsr); Statement stat = getStatement(connection, queryTimeout); ResultSet rs = stat.executeQuery(dsr.getQuery())) {
            ...
            // 若维度和指标的字段数小于结果集的字段数，则去掉结果集中的时间字段
            int rowLength = list.size() > 0 ? list.get(0).length : 0;
            if (list.size() > 0 && dsr.getXAxis() != null && dsr.getYAxis() != null && rowLength > dsr.getXAxis().size() + dsr.getYAxis().size()) {
                List<String[]> newlist = new LinkedList<>();
                for (int i = 0; i < list.size(); i++) {
                    String[] row = list.get(i);
                    String[] newRow = new String[rowLength - 1];
                    for (int i1 = 0; i1 < newRow.length; i1++) {
                        newRow[i1] = row[i1 + 1];
                    }
                    newlist.add(newRow);
                }
            ...
            if (list.size() > 0 && dsr.getXAxis() != null && dsr.getYAxis() != null && rowLength == dsr.getXAxis().size() + dsr.getYAxis().size()) {
                int index = 0;
                for (int j = 0; j < dsr.getXAxis().size(); j++) {
                    if (dsr.getXAxis().get(j).getName().equals("Time")) {
                        index = j;
                    }
                }
...
//    @Override
    public Connection getConnection2(DatasourceRequest datasourceRequest) throws Exception  {
            ...
            for (int j = 0; j < columnCount; j++) {
                ...
                switch (columnType) {
                    case Types.DATE:
                        ...
                        break;
                    case Types.FLOAT:
                        row[j] = String.valueOf(rs.getFloat(j + 1));
                        break;
                    case Types.BOOLEAN:
