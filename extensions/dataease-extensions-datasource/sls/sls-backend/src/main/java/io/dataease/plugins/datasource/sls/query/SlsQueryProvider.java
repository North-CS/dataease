...
//        }
        ...
        if (CollectionUtils.isNotEmpty(fields)) {
            for (int i = 0; i < fields.size(); i++) {
                ...
                if (StringUtils.equals(fieldAlias, "__time__")) {
                    fieldAlias = String.format(SlsConstants.ALIAS_FIX, String.format(SQLConstants.FIELD_ALIAS_X_PREFIX, i));
                }
                String fieldName = "";
                // 处理横轴字段
                if (f.getDeExtractType() == 1) {
    ...
    public String createQuerySQLAsTmp(String sql, List<DatasetTableField> fields, boolean isGroup,
                                      FilterTreeObj fieldCustomFilter, List<DataSetRowPermissionsTreeDTO> rowPermissionsTree) {
            ...
            for (int i = 0; i < fields.size(); i++) {
                ...
                if (f.getDeExtractType() == DeTypeConstants.DE_TIME) {
                    if (f.getDeType() == DeTypeConstants.DE_INT || f.getDeType() == DeTypeConstants.DE_FLOAT) {
                        fieldName = String.format(SlsConstants.UNIX_TIMESTAMP, originField);
                    } else {
                        fieldName = originField;
                    }
                    ...
                    if (f.getDeType() == DeTypeConstants.DE_INT) {
                        ...
                        fieldName = String.format(SlsConstants.STR_TO_DATE, originField,
                    ...
                    if (f.getDeType() == DeTypeConstants.DE_TIME) {
                        String cast = String.format(SlsConstants.CAST, originField, "bigint");
                        fieldName = String.format(SlsConstants.FROM_UNIXTIME, cast);
    ...
    public String createQueryTableWithPage(String table, List<DatasetTableField> fields, Integer page, Integer pageSize,
                                           Integer realSize, boolean isGroup, Datasource ds, FilterTreeObj fieldCustomFilter, List<DataSetRowPermissionsTreeDTO> rowPermissionsTree) {
    ...
    public String createQuerySQLWithPage(String sql, List<DatasetTableField> fields, Integer page, Integer pageSize,
                                         Integer realSize, boolean isGroup, FilterTreeObj fieldCustomFilter, List<DataSetRowPermissionsTreeDTO> rowPermissionsTree) {
    ...
    public String createQueryTableWithLimit(String table, List<DatasetTableField> fields, Integer limit,
                                            boolean isGroup, Datasource ds, FilterTreeObj fieldCustomFilter, List<DataSetRowPermissionsTreeDTO> rowPermissionsTree) {
    ...
    public String getSQL(String table, List<ChartViewFieldDTO> xAxis, List<ChartViewFieldDTO> yAxis,
                         FilterTreeObj fieldCustomFilter, List<DataSetRowPermissionsTreeDTO> rowPermissionsTree, List<ChartExtFilterRequest> extFilterRequestList,
        ...
        if (CollectionUtils.isNotEmpty(xAxis)) {
            for (int i = 0; i < xAxis.size(); i++) {
                ...
                if (i == baseXAxis.size()) {// 起止时间
                    String fieldName = String.format(SlsConstants.AGG_FIELD, "min", originField);
                    yFields.add(getXFields(x, fieldName, fieldAlias));

                    yWheres.add(getYWheres(x, originField, fieldAlias));

                } else if (i == baseXAxis.size() + 1) {
                    String fieldName = String.format(SlsConstants.AGG_FIELD, "max", originField);
