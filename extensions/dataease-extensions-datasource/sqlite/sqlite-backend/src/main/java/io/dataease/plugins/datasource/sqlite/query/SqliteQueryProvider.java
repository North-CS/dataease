package io.dataease.plugins.datasource.sqlite.query;
...
public class SqliteQueryProvider extends QueryProvider {
    @Resource
    ...
    public String createQuerySQL(String table, List<DatasetTableField> fields, boolean isGroup, Datasource ds, FilterTreeObj fieldCustomFilter, List<DataSetRowPermissionsTreeDTO> rowPermissionsTree, List<DeSortField> sortFields, Long limit, String keyword) {
        SQLObj tableObj = SQLObj.builder()
                .tableName((table.startsWith("(") && table.endsWith(")")) ? table : String.format(SqliteConstants.KEYWORD_TABLE, table))
        ...
        if (CollectionUtils.isNotEmpty(fields)) {
            for (int i = 0; i < fields.size(); i++) {
                ...
                if (ObjectUtils.isNotEmpty(f.getExtField()) && f.getExtField() == 2) {
                    ...
                    originField = String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(), f.getOriginName());
                } else {
                    originField = String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(), f.getOriginName());
                }
                ...
                if (f.getDeExtractType() == 1) {
                    if (f.getDeType() == 2 || f.getDeType() == 3) {
                        fieldName = String.format(SqliteConstants.UNIX_TIMESTAMP, originField) + "*1000";
                    } else {
                        fieldName = originField;
                    }
                } else if (f.getDeExtractType() == 0) {
                    if (f.getDeType() == 2) {
                        fieldName = String.format(SqliteConstants.CAST, originField, SqliteConstants.DEFAULT_INT_FORMAT);
                    } else if (f.getDeType() == 3) {
                        fieldName = String.format(SqliteConstants.CAST, originField, SqliteConstants.DEFAULT_FLOAT_FORMAT);
                    } else if (f.getDeType() == 1) {
                        fieldName = StringUtils.isEmpty(f.getDateFormat()) ? String.format(SqliteConstants.STR_TO_DATE, SqliteConstants.DEFAULT_DATE_FORMAT, originField) :
                                String.format(SqliteConstants.DATE_FORMAT, SqliteConstants.DEFAULT_DATE_FORMAT, String.format(SqliteConstants.STR_TO_DATE, f.getDateFormat(), originField));
                    } else {
                        fieldName = originField;
                    }
                } else {
                    if (f.getDeType() == 1) {
                        String cast = String.format(SqliteConstants.CAST, originField, SqliteConstants.DEFAULT_INT_FORMAT) + "/1000";
                        fieldName = String.format(SqliteConstants.FROM_UNIXTIME, SqliteConstants.DEFAULT_DATE_FORMAT, cast);
...
//                  解析origin name中有关联的字段生成sql表达式
                    ...
                    originField = String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName());
                } else {
                    originField = String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName());
                }
                String fieldAlias = String.format(SQLConstants.FIELD_ALIAS_X_PREFIX, i);
        ...
        if (CollectionUtils.isNotEmpty(yAxis)) {
            for (int i = 0; i < yAxis.size(); i++) {
                ...
                if (ObjectUtils.isNotEmpty(y.getExtField()) && y.getExtField() == 2) {
                    ...
                    originField = String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(), y.getOriginName());
                } else {
                    originField = String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(), y.getOriginName());
                }
        ...
        SQLObj tableSQL = SQLObj.builder()
                .tableName(String.format(SqliteConstants.BRACKETS, sql))
            ...
            for (int i = 0; i < xAxis.size(); i++) {
                ...
                if (ObjectUtils.isNotEmpty(x.getExtField()) && x.getExtField() == 2) {
                    ...
                    if (x.getDeType() == 2 || x.getDeType() == 3) {
                        originField = String.format(SqliteConstants.CAST, String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName()), SqliteConstants.DEFAULT_FLOAT_FORMAT);
                    } else {
                        originField = String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName());
                    }
                }
...
//    public String getSQLScatter(String table, List<ChartViewFieldDTO> xAxis, List<ChartViewFieldDTO> yAxis, FilterTreeObj fieldCustomFilter, List<DataSetRowPermissionsTreeDTO> rowPermissionsTree, List<ChartExtFilterRequest> extFilterRequestList, List<ChartViewFieldDTO> extBubble, Datasource ds, ChartViewWithBLOBs view) {
        SQLObj tableObj = SQLObj.builder()
                .tableName((table.startsWith("(") && table.endsWith(")")) ? table : String.format(SqliteConstants.KEYWORD_TABLE, table))
                .tableAlias(String.format(TABLE_ALIAS_PREFIX, 0))
                .build();
    ...
    public String getSQLRangeBar(String table, List<ChartViewFieldDTO> baseXAxis, List<ChartViewFieldDTO> xAxis, List<ChartViewFieldDTO> yAxis, FilterTreeObj fieldCustomFilter, List<DataSetRowPermissionsTreeDTO> rowPermissionsTree, List<ChartExtFilterRequest> extFilterRequestList, List<ChartViewFieldDTO> extStack, Datasource ds, ChartViewWithBLOBs view) {
        SQLObj tableObj = SQLObj.builder()
                .tableName((table.startsWith("(") && table.endsWith(")")) ? table
                        : String.format(SqliteConstants.KEYWORD_TABLE, table))
                .tableAlias(String.format(SqliteConstants.ALIAS_FIX, String.format(TABLE_ALIAS_PREFIX, 0)))
                .build();
        setSchema(tableObj, ds);
        List<SQLObj> xFields = new ArrayList<>();
        List<SQLObj> xOrders = new ArrayList<>();
            ...
            for (int i = 0; i < xAxis.size(); i++) {
                ...
                if (StringUtils.equalsIgnoreCase(x.getGroupType(), "q")) {
                    if (ObjectUtils.isNotEmpty(x.getExtField()) && x.getExtField() == 2) {
                        ...
                        originField = String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(),
                                x.getOriginName());
                    } else {
                        originField = String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(),
                                x.getOriginName());
                    }
                    String fieldAlias = String.format(SqliteConstants.ALIAS_FIX,
                ...
                if (i == baseXAxis.size()) {// 起止时间
                    String fieldName = String.format(SqliteConstants.AGG_FIELD, "min", originField);
                    yFields.add(getXFields(x, fieldName, fieldAlias));

                    yWheres.add(getYWheres(x, originField, fieldAlias));

                } else if (i == baseXAxis.size() + 1) {
                    String fieldName = String.format(SqliteConstants.AGG_FIELD, "max", originField);
    ...
    public String transTreeItem(SQLObj tableObj, DatasetRowPermissionsTreeItem item) {
        ...
        if (ObjectUtils.isNotEmpty(field.getExtField()) && field.getExtField() == 2) {
            ...
            originName = String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(), field.getOriginName());
        } else {
            originName = String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(), field.getOriginName());
        }
        ...
        if (field.getDeType() == 1) {
            if (field.getDeExtractType() == 0 || field.getDeExtractType() == 5 || field.getDeExtractType() == 1) {
                whereName = String.format(SqliteConstants.STR_TO_DATE, StringUtils.isNotEmpty(field.getDateFormat()) ? field.getDateFormat() : SqliteConstants.DEFAULT_DATE_FORMAT, originName);
            }
            if (field.getDeExtractType() == 2 || field.getDeExtractType() == 3 || field.getDeExtractType() == 4) {
                String cast = String.format(SqliteConstants.CAST, originName, SqliteConstants.DEFAULT_INT_FORMAT) + "/1000";
                whereName = String.format(SqliteConstants.FROM_UNIXTIME, SqliteConstants.DEFAULT_DATE_FORMAT, cast);
            }
        } else if (field.getDeType() == 2 || field.getDeType() == 3) {
            if (field.getDeExtractType() == 0 || field.getDeExtractType() == 5) {
                whereName = String.format(SqliteConstants.CAST, originName, SqliteConstants.DEFAULT_FLOAT_FORMAT);
            }
            if (field.getDeExtractType() == 1) {
                whereName = String.format(SqliteConstants.UNIX_TIMESTAMP, originName) + "*1000";
            }
            if (field.getDeExtractType() == 2 || field.getDeExtractType() == 3 || field.getDeExtractType() == 4) {
                whereName = originName;
            }
        } else if (field.getDeType() == 0 && field.getDeExtractType() == 0) {
            whereName = String.format(SqliteConstants.CAST, originName, SqliteConstants.CHAR);
        } else {
            whereName = originName;
        }
        ...
        if (StringUtils.equalsIgnoreCase(item.getFilterType(), "enum")) {
            ...
            if (StringUtils.equalsIgnoreCase(item.getTerm(), "null")) {
                ...
                whereValue = String.format(SqliteConstants.WHERE_VALUE_VALUE, value);
            }
    ...
    public String convertTableToSql(String tableName, Datasource ds) {
        return createSQLPreview("SELECT * FROM " + String.format(SqliteConstants.KEYWORD_TABLE, tableName), null);
    }
            ...
            if (StringUtils.equalsIgnoreCase(request.getFilterType(), "enum")) {
                ...
                for (ChartCustomFilterItemDTO filterItemDTO : filter) {
                    ...
                    if (StringUtils.equalsIgnoreCase(filterItemDTO.getTerm(), "null")) {
                        ...
                        /*if (field.getDeExtractType() == 2 || field.getDeExtractType() == 3 || field.getDeExtractType() == 4) {
                            whereValue = String.format(SqliteConstants.WHERE_NUMBER_VALUE, value);
                        } else {
                            whereValue = String.format(SqliteConstants.WHERE_VALUE_VALUE, value);
                        }*/
                        whereValue = String.format(SqliteConstants.WHERE_VALUE_VALUE, value);
        ...
        for (ChartExtFilterRequest request : requestList) {
            ...
            for (DatasetTableField field : fieldList) {
                ...
                if (field.getDeType() == 1) {
                    ...
                    if (field.getDeExtractType() == 0 || field.getDeExtractType() == 5 || field.getDeExtractType() == 1) {
                        String date = String.format(SqliteConstants.STR_TO_DATE, StringUtils.isNotEmpty(field.getDateFormat()) ? field.getDateFormat() : SqliteConstants.DEFAULT_DATE_FORMAT, originName);
                        if(request.getOperator().equals("between")){
                            whereName = date;
                        }else {
                            whereName = String.format(SqliteConstants.DATE_FORMAT, format, date);
                        }
                    }
                    if (field.getDeExtractType() == 2 || field.getDeExtractType() == 3 || field.getDeExtractType() == 4) {
                        if(request.getOperator().equals("between")){
            ...
            if (StringUtils.containsIgnoreCase(request.getOperator(), "in")) {
                ...
                if (request.getDatasetTableField().getDeType() == 1) {
                    if (request.getDatasetTableField().getDeExtractType() == 2
                        ...
                        whereValue = String.format(SqliteConstants.WHERE_BETWEEN, value.get(0), value.get(1));
                    } else {
                        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                        String startTime = simpleDateFormat.format(new Date(Long.parseLong(value.get(0))));
                        String endTime = simpleDateFormat.format(new Date(Long.parseLong(value.get(1))));
                        whereValue = String.format(SqliteConstants.WHERE_BETWEEN, startTime, endTime);
                    }
                } else {
                    whereValue = String.format(SqliteConstants.WHERE_BETWEEN, value.get(0), value.get(1));
                }
            } else {
                whereValue = String.format(SqliteConstants.WHERE_VALUE_VALUE, value.get(0));
            }
            list.add(SQLObj.builder()
                    .whereField(whereName)
                    .whereTermAndValue(whereTerm + whereValue)
        ...
        if (x.getDeExtractType() == 1) {
            if (x.getDeType() == 2 || x.getDeType() == 3) {
                ...
                if (StringUtils.equalsIgnoreCase(x.getDateStyle(), "y_Q")) {
                    fieldName = String.format(format,
                            String.format(SqliteConstants.DATE_FORMAT, "%Y", originField),
                            String.format(MysqlConstants.QUARTER, originField));
                } else {
                    fieldName = String.format(SqliteConstants.DATE_FORMAT, format, originField);
                }
            } else {
                fieldName = originField;
            }
            ...
            if (x.getDeType() == 1) {
                ...
                if (x.getDeExtractType() == 0) {
                    if (StringUtils.equalsIgnoreCase(x.getDateStyle(), "y_Q")) {
                        fieldName = String.format(format,
                                String.format(SqliteConstants.DATE_FORMAT, "%Y", String.format(SqliteConstants.STR_TO_DATE, SqliteConstants.DEFAULT_DATE_FORMAT, originField)),
                                String.format(MysqlConstants.QUARTER, String.format(SqliteConstants.STR_TO_DATE, SqliteConstants.DEFAULT_DATE_FORMAT, originField)));
                    } else {
                        fieldName = String.format(SqliteConstants.DATE_FORMAT, format, originField);
                    }
                } else {
                    String cast = String.format(SqliteConstants.CAST, originField, SqliteConstants.DEFAULT_INT_FORMAT) + "/1000";
                    String from_unixtime = String.format(SqliteConstants.FROM_UNIXTIME, SqliteConstants.DEFAULT_DATE_FORMAT, cast);
                    if (StringUtils.equalsIgnoreCase(x.getDateStyle(), "y_Q")) {
                        fieldName = String.format(format,
                                String.format(SqliteConstants.DATE_FORMAT, "%Y", from_unixtime),
                                String.format(MysqlConstants.QUARTER, from_unixtime));
                    } else {
                        fieldName = String.format(SqliteConstants.DATE_FORMAT, format, from_unixtime);
                    }
                }
            } else if (x.getDeType() == 0 && x.getDeExtractType() == 0) {
                fieldName = String.format(SqliteConstants.CAST, originField, SqliteConstants.CHAR);
            } else {
        ...
        if (CollectionUtils.isNotEmpty(x.getFilter()) && x.getFilter().size() > 0) {
            x.getFilter().forEach(f -> {
                ...
                if (StringUtils.equalsIgnoreCase(f.getTerm(), "null")) {
                    ...
                    whereValue = String.format(SqliteConstants.WHERE_VALUE_VALUE, f.getValue());
                }
                list.add(SQLObj.builder()
                        .whereField(whereName)
    ...
    private SQLObj getYFields(ChartViewFieldDTO y, String originField, String fieldAlias) {
        ...
        if (StringUtils.equalsIgnoreCase(y.getOriginName(), "*")) {
            fieldName = SqliteConstants.AGG_COUNT;
        } else if (SQLConstants.DIMENSION_TYPE.contains(y.getDeType())) {
            if (StringUtils.equalsIgnoreCase(y.getSummary(), "count_distinct")) {
                fieldName = String.format(SqliteConstants.AGG_FIELD, "COUNT", "DISTINCT " + originField);
            } else {
                fieldName = String.format(SqliteConstants.AGG_FIELD, y.getSummary(), originField);
            }
        } else {
            if (StringUtils.equalsIgnoreCase(y.getSummary(), "avg") || StringUtils.containsIgnoreCase(y.getSummary(), "pop")) {
                String cast = String.format(SqliteConstants.CAST, originField, y.getDeType() == 2 ? SqliteConstants.DEFAULT_INT_FORMAT : SqliteConstants.DEFAULT_FLOAT_FORMAT);
                String agg = String.format(SqliteConstants.AGG_FIELD, y.getSummary(), cast);
                String cast1 = String.format(SqliteConstants.CAST, agg, SqliteConstants.DEFAULT_FLOAT_FORMAT);
                fieldName = String.format(SqliteConstants.ROUND, cast1, "8");
            } else {
                String cast = String.format(SqliteConstants.CAST, originField, y.getDeType() == 2 ? SqliteConstants.DEFAULT_INT_FORMAT : SqliteConstants.DEFAULT_FLOAT_FORMAT);
                if (StringUtils.equalsIgnoreCase(y.getSummary(), "count_distinct")) {
                    fieldName = String.format(SqliteConstants.AGG_FIELD, "COUNT", "DISTINCT " + cast);
                } else {
                    fieldName = String.format(SqliteConstants.AGG_FIELD, y.getSummary(), cast);
                }
            }
        }
        return SQLObj.builder()
    ...
    private String calcFieldRegex(String originField, SQLObj tableObj) {
        ...
        for (DatasetTableField ele : calcFields) {
            originField = originField.replaceAll("\\[" + ele.getId() + "]",
                    String.format(SqliteConstants.KEYWORD_FIX, tableObj.getTableAlias(), ele.getOriginName()));
        }
        return originField;
    }
